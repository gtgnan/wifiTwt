...
// The STA offers uplink video traffic at configured video quality. PHY rate is set to HE MCS 7 with 20 MHz bandwidth and 800 ns GI.
...
// Run the shell script wns3DutyCycleScript.sh to generate the raw results found in logs/wns3DutyCycleResults.log
// Run the Jupyter notebook logs/wns3DutyCyclePlotting.ipynb to process the raw results and generate plots

// Usage example: ./ns3 run "wns3DutyCycle --simId=1000 --randSeed=1000 --simulationTime=40 --dutyCycle=0.5 --twtWakeIntervalMultiplier=1 --videoQuality=1 --parallelSim=false"
...
double dutyCycle = 0.1;    // Duty cycle for TWT - 1 corresponds to no TWT (CAM)
// std::string useCase = "noTwt";    // Use TWT


std::string scenario = "wns3DutyCycle";    // used for logging and processing. Get from command line argument
...
int main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("dutyCycle", "double 0 to 1; 1 corresponds to CAM", dutyCycle);
  ...
  {
    useCase = "twt";
    // TWT Timing parameters - based on dutyCycle
    twtNominalWakeDurationDivider = 1/(dutyCycle*twtWakeIntervalMultiplier);    // K, where twtNomimalWakeDuration = BI / K; K is double
  }
...
//   Time AdvanceWakeupPS = MicroSeconds (10);
  ...
  Config::SetDefault ("ns3::WifiMacQueue::MaxDelay", TimeValue (MilliSeconds (1000)));    // Default is 500 ms - changed to 1000ms because frames timed out at high load and TCP connection kept dropping. 
  ...
  */
  ...
  for (std::size_t i = 0; i < nStations; i++)
  {
    ...
    <<"; Configured Duty cycle = "<<dutyCycle
  ...
  if (parallelSim)
  {
    ssNodeLog <<LOG_PATH<< "wns3DutyCycleResults"<<currentsimId_string<<".log";
  }
  else
  {
    ssNodeLog <<LOG_PATH<< "wns3DutyCycleResults.log";  
  }
  // static std::fstream g (ssNodeLog.str ().c_str (), std::ios::app);
