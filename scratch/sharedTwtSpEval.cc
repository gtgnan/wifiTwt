...
int accordionLevel = 1;    // Level of the binary tree to which TWT schedules will be split into. 0 for no Twt, 1 for 2 TWT SPs per period, 2 for 4 TWT SPs, 3 for 8 TWT SPs, 4 for 16 TWT SPs and so on. STAs will be equally split between SPs
// std::string useCase = "noTwt";    // Use TWT


std::string scenario = "TWT_dedicated";    // used for logging and processing. Get from command line argument
// std::string scenario = "CAM";   
...
std::size_t nStations {4};              // number of stations
double simulationTime {10 + 10};        //seconds  - 10 seconds for setup
...
// CBR UDP traffic configuration
bool enableCbrUplink = true;
bool enableCbrDownlink = false;
uint32_t cbrUlDatarateKbps = 500;  // kbps
uint32_t cbrDlDatarateKbps = 500;  // kbps
...
double twtWakeIntervalMultiplier, twtNominalWakeDurationDivider, nextStaTwtSpOffsetDivider;
uint32_t staCountModulusForTwt;    // Offset for next STA TWT start is added only after each staCountModulusForTwt STAs
...
initiateTwtAtApAndSta (Ptr<WifiMac> apMac, Ptr<WifiMac> staMac, u_int8_t flowId, Time twtWakeInterval, Time twtNominalWakeDuration, Time nextTwtOffsetFromNextBeacon)
{
  ...
  // apMac -> SetTwtSchedule (flowId, staMacAddress, false, true, true, false, true, 0, twtWakeInterval, twtNominalWakeDuration, MilliSeconds(10)); 
}
//-*************************************************************************
...
int main (int argc, char *argv[])
{
  ...
  // cmd.AddValue ("protocol", "Transport protocol",protocol);
  ...
  cmd.AddValue ("enableCbrUplink", "CBR Uplink enabled if true", enableCbrUplink);
  cmd.AddValue ("enableCbrDownlink", "CBR downlink enabled if true", enableCbrDownlink);
  cmd.AddValue ("cbrUlDatarateKbps", "CBR Uplink datarate", cbrUlDatarateKbps);
  cmd.AddValue ("cbrDlDatarateKbps", "CBR Downlink datarate", cbrDlDatarateKbps);
  ...
  cmd.AddValue ("accordionLevel", "log2 (nSta)>=Integer >=0; Level of the binary tree to split for Accordion scheduling", accordionLevel);
  ...
  // nStations should be a power of 2
  NS_ASSERT_MSG ((nStations & (nStations - 1)) == 0, "nStations should be a power of 2");
  NS_ASSERT_MSG (nStations > 0, "nStations should be > 0");
  // accordionLevel should be less than or equal to log2(nStations)
  NS_ASSERT_MSG (accordionLevel <= log2(nStations), "accordionLevel should be less than or equal to log2(nStations)");
  ...
  {
    useCase = "twt";
    // TWT Timing parameters - based on accordionLevel
    // twtWakeIntervalMultiplier = 1;    // K, where twtWakeInterval = BI * K; K is double
    twtWakeIntervalMultiplier = 1;    // K, where twtWakeInterval = BI * K; K is double
    // double twtNominalWakeDurationDivider = nStations;    // K, where twtNomimalWakeDuration = BI / K; K is double
    twtNominalWakeDurationDivider = pow(2, accordionLevel);    // K, where twtNomimalWakeDuration = BI / K; K is double
    nextStaTwtSpOffsetDivider = pow(2, accordionLevel);   // K, where nextStaTwtSpOffset = BI / K; K is double
    staCountModulusForTwt = nStations/pow(2, accordionLevel);      // Modulus for STA count for TWT SP start - TWT SP start Offset is added only after each 'staCountModulusForTwt' STAs
  }
...
//   Time AdvanceWakeupPS = MicroSeconds (10);
  ...
  Config::SetDefault ("ns3::WifiMacQueue::MaxDelay", TimeValue (MilliSeconds (500)));    // Default is 500 ms - changed because frames timed out and TCP connection kept dropping. 
  ...
  */
  ...
  if (useCase == "twt")
  {
    ...
    {
      u_int32_t offsetInt = i/staCountModulusForTwt;
      
      
      Ptr<WifiNetDevice> apDevice = DynamicCast<WifiNetDevice>(ApNode->GetDevice(1));
      // Get ap mac object from apDevice->GetMac()
      ...
      Time nextTwtOffsetFromNextBeacon = offsetInt*(beaconIntervalEffective/nextStaTwtSpOffsetDivider) + firstTwtSpOffsetFromBeacon;
      ...
      // Simulator::Schedule (firstTwtSpStart, &initiateTwtAtApAndSta, apMac, staMac, 0, beaconInterval*twtWakeIntervalMultiplier, beaconInterval/twtNominalWakeDurationDivider, offsetInt*(beaconInterval/nextStaTwtSpOffsetDivider) + firstTwtSpOffsetFromBeacon);
      ...
      // apMac -> SetTwtSchedule (0, staMacAddress, false, true, true, false, true, 0, beaconInterval*twtWakeIntervalMultiplier, beaconInterval/twtNominalWakeDurationDivider, offsetInt*(beaconInterval/nextStaTwtSpOffsetDivider));   
  ...
  if (enableCbrUplink)
  {
    ...
    // Uplink CBR traffic setup - UDP is used
    uint16_t port = 50000;
    for (std::size_t i = 0; i < nStations; i++)
    {
      ...
      // Client Apps
      OnOffHelper onOffHelper ("ns3::UdpSocketFactory", (InetSocketAddress (p2pNodeInterfaces.GetAddress (1), port+i)));
      onOffHelper.SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=1]"));
      onOffHelper.SetAttribute ("OffTime", StringValue ("ns3::ConstantRandomVariable[Constant=0]"));
      std::ostringstream datarateString;
      datarateString << cbrUlDatarateKbps << "kbps";
      onOffHelper.SetAttribute ("DataRate", StringValue(datarateString.str()));
      // onOffHelper.SetAttribute ("DataRate", StringValue("2kbps"));
      onOffHelper.SetAttribute ("PacketSize", UintegerValue (payloadSize));
      ApplicationContainer clientApp = onOffHelper.Install (wifiStaNodes.Get (i));
      
      
      clientApp.Start (MilliSeconds (thisAppStartTime_ms->GetValue ()));
      // clientApp.Start (AppStartTimeMin);
      clientApp.Stop (Seconds (simulationTime));
    }
  }

  // Setting up CBR UDP DL traffic
  if (enableCbrDownlink)
  {
    // Generate a random time between AppStartTimeMin and AppStartTimeMax
    Ptr<UniformRandomVariable> thisAppStartTime_ms = CreateObject<UniformRandomVariable> ();
    thisAppStartTime_ms->SetAttribute ("Min", DoubleValue (AppStartTimeMin.GetMilliSeconds()));
    thisAppStartTime_ms->SetAttribute ("Max", DoubleValue (AppStartTimeMax.GetMilliSeconds()));
    // Downlink CBR traffic setup - UDP is used
    uint16_t port = 60000;
    for (std::size_t i = 0; i < nStations; i++)
    {
      ...
      // Client Apps
      OnOffHelper onOffHelper ("ns3::UdpSocketFactory", (InetSocketAddress (staNodeInterfaces.GetAddress(i), port+i)));
      onOffHelper.SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=1]"));
      onOffHelper.SetAttribute ("OffTime", StringValue ("ns3::ConstantRandomVariable[Constant=0]"));
      std::ostringstream datarateString;
      datarateString << cbrDlDatarateKbps << "kbps";
      onOffHelper.SetAttribute ("DataRate", StringValue(datarateString.str()));
      // onOffHelper.SetAttribute ("DataRate", StringValue("2kbps"));
      onOffHelper.SetAttribute ("PacketSize", UintegerValue (payloadSize));
      ApplicationContainer clientApp = onOffHelper.Install (p2pServerNode);
  ...
  if (enableFlowMon)
  {
    ...
    for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
      ...
      if (ipToMac.find(t.destinationAddress) != ipToMac.end())
      {
        ...
        staMacToUplinkDownlink90Latency_us[ipToMac[t.sourceAddress]].second = latency90 * 1e6;
        staMacToUplinkDownlink95Latency_us[ipToMac[t.sourceAddress]].second = latency95 * 1e6;
        staMacToUplinkDownlink99Latency_us[ipToMac[t.sourceAddress]].second = latency99 * 1e6;
      }
      if (throughputKbps == 0)
      {
  ...
  ssNodeLog <<LOG_PATH<< "logAllperSta"<<currentLoopIndex_string<<".log";
  // static std::fstream g (ssNodeLog.str ().c_str (), std::ios::app);
  ...
  else 
  {
    for (auto it = staMacToTotalBitsUplinkDownlink.begin (); it != staMacToTotalBitsUplinkDownlink.end (); it++)
    {
      ...
      <<";nSTA="<<nStations
      <<";cbrUlDatarateKbps_PerSta="<<cbrUlDatarateKbps
  ...
  std::cout<<std::setw(30) << std::right << "accordionLevel" << ":\t" << +accordionLevel << std::endl;
