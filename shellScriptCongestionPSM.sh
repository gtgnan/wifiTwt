...
outputFileName=script_logs/consoleOutputApr26_5StaPsm50100loadTcp.txt		# changeThis
logFileName=script_logs/scriptLogApr26_5StaPsm50100loadTcp.txt			# changeThis
loopIndex=80000		# changeThis
otherStaCount=5		# changeThis


CURRENTDATE=`date +"%Y-%m-%d %T"`
echo "New Script started at ${CURRENTDATE}" >> ${outputFileName}
echo "loopIndex, simulationTime, enablePSM_flag, enableTcpUplink, dataPeriod, otherStaCount, dataRatebps_other,enableMulticast, multicastInterval_ms, randSeed" >> ${logFileName}


# simulationTime=30
simulationTime=30
enablePSM_flag=true
enableTcpUplink=true
dataPeriod=1
...
do
	...
	do
		...
		echo "${loopIndex}, ${simulationTime}, ${enablePSM_flag},  ${enableTcpUplink}, ${dataPeriod}, ${otherStaCount}, ${dataRatebps_other_full}, ${enableMulticast}, ${multicastInterval_ms}, ${randSeed}" >> ${logFileName}
		echo "--loopIndex=${loopIndex} --simulationTime=${simulationTime} --enablePSM_flag=${enablePSM_flag} --otherStaCount=${otherStaCount} --enableTcpUplink=${enableTcpUplink} --dataPeriod=${dataPeriod} --dataRatebps_other=${dataRatebps_other_full} --enableMulticast=${enableMulticast} --multicastInterval_ms=${multicastInterval_ms} --randSeed=${randSeed}"
