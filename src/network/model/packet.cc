          ...
          switch (item.type) {
            ...
            case PacketMetadata::Item::TRAILER:
              os << item.tid.GetName () << " (";
              {
                ...
                // shyam --------------------------------------------------------------------------------------- #fixAsap
                if (item.tid.GetName () == "ns3::CtrlBAckResponseHeader")
                {
                  NS_LOG_WARN ("Bug with Packet.Print() for ns3::CtrlBAckResponseHeader. Skipping for now.");
                  chunk->Print (os);
                  delete chunk;
                  break;
                }
                ...
                if (item.type == PacketMetadata::Item::HEADER)
                  {
                    ...
                    // shyam - bug here for printing multi sta B Ack #fixAsap
