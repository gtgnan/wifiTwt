...
HeFrameExchangeManager::StartFrameExchange(Ptr<QosTxop> edca, Time availableTime, bool initialFrame)
{
    ...
    {
        mpdu = edca->PeekNextMpdu(m_linkId);
        // Check if recipient MAC address has a TWT agreement. If yes, check if endSp is running. If yes, find time till endSp event and set that as availableTime
        if (mpdu && mpdu->GetHeader().IsQosData() && !mpdu->GetHeader().GetAddr1().IsGroup())
        {
            Mac48Address recipient = mpdu->GetHeader().GetAddr1();
            if (GetWifiRemoteStationManager()->GetTwtAgreementCount(recipient) > 0)
            {
                // set availableTime to minimum of availableTime and time till endSp event
                Time endSpTime = GetWifiRemoteStationManager()->GetTimeTillEndOfOngoingTwtSPs(recipient);
                NS_LOG_DEBUG ("End SP time for recipient " << recipient << " is " << endSpTime << " availableTime is " << availableTime);
                if (availableTime.IsStrictlyPositive())
                {
                    availableTime = std::min(availableTime, endSpTime);
                }
                else
                {
                    availableTime = endSpTime;
                }
            }
        }
