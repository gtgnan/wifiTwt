...
void
HeFrameExchangeManager::SendTwtSetupFrame (WifiTwtAgreement agreement)
{
  // Sends the TWT agreement as a TWT Setup frame (accept)
  NS_LOG_FUNCTION (this << agreement);
  WifiMacHeader hdr;
  // hdr.SetType (WIFI_MAC_MGT_ACTION_NO_ACK);    //TODO
  hdr.SetType (WIFI_MAC_MGT_ACTION);
  hdr.SetAddr1 (agreement.GetPeerMacAddress());
  hdr.SetAddr2 (m_self);
  ...
  action.unprotectedS1GAction = WifiActionHeader::TWT_SETUP;
  actionHdr.SetAction (WifiActionHeader::UNPROTECTED_S1G, action);

  Ptr<Packet> packet = Create<Packet> ();
  MgtTwtSetupHeader twtSetupHdr;

  // Creating the TWT element to be added to the TWT mgt action frame
  Twt twtElement;
  twtElement.SetNdpPagingIndicator (false);
  twtElement.SetResponderPm (false);
  twtElement.SetNegotiationType (!agreement.IsIndividualAgreement());
  twtElement.SetTwtInfoFrameDisabled (true);
  twtElement.SetWakeDurationUnit (0);    // 0 for 256 us, 1 for 1024 us
  twtElement.SetTwtRequest(false);   // 0 for AP
  twtElement.SetTwtSetupCommand (4); // 4 for TWT Setup Accept
  twtElement.SetTrigger (agreement.IsTriggerBasedAgreement());
  twtElement.SetImplicit (agreement.IsImplicitAgreement());
  twtElement.SetFlowType (agreement.GetFlowType());
  twtElement.SetFlowId (agreement.GetFlowId());
  // wake interval exponent calculation
  uint64_t wakeInterval_us = agreement.GetWakeInterval().GetMicroSeconds();
  NS_LOG_DEBUG ("Initial wakeInterval_us = " << wakeInterval_us);
  // if wakeInterval_us will fit in 2 octets, set exponent to 0; else divide by 2 until it fits
  uint8_t wakeIntervalExponent = 0;
  while (wakeInterval_us > 65535)
  {
    wakeInterval_us = wakeInterval_us / 2;
    NS_LOG_DEBUG ("updated wakeInterval_us = " << wakeInterval_us);
    wakeIntervalExponent++;
  }
  NS_ASSERT_MSG (wakeIntervalExponent <= 31, "Wake interval exponent must be <= 63. Wake interval too large");
  twtElement.SetTwtWakeIntervalExponent (wakeIntervalExponent);  
  twtElement.SetTwtProtection (false);
  twtElement.SetTargetWakeTime (agreement.GetNextTwt().GetMicroSeconds());
  twtElement.SetTwtGroupAssignment (0);
  twtElement.SetNominalWakeDuration (uint8_t(agreement.GetNominalWakeDuration().GetMicroSeconds()/256));
  NS_LOG_DEBUG ("Setting wakeInterval_us = " << uint16_t (wakeInterval_us));
  twtElement.SetTwtWakeIntervalMantissa (uint16_t(wakeInterval_us));
  twtElement.SetTwtChannel (0);

  // Adding to mgt action frame
  twtSetupHdr.SetTwt (twtElement);


  // print ostream from twtSetupHdr.Print
  std::stringstream ss;
  twtSetupHdr.Print (ss);
  NS_LOG_DEBUG (ss.str());
  
  packet->AddHeader (twtSetupHdr);
  ...
  m_mac->GetQosTxop (AC_BE)->GetWifiMacQueue ()->PushFront (mpdu);
...
HeFrameExchangeManager::StartFrameExchange (Ptr<QosTxop> edca, Time availableTime, bool initialFrame)
{
  ...
  //TODO: check if there is a TWT setup/teardown frame to be transmitted
  // For now, TWT setup/teardown will only be sent in SU_TX mode
  if (GetPendingTwtAgreementsCount() > 0)
  {
    // Dequeue and send one TWT Accept Mgt frame
    WifiTwtAgreement agreement = DequeueFirstPendingTwtAgreement();
    NS_LOG_DEBUG ("TWT Agreement found in HeFeMan: " << agreement);
    SendTwtSetupFrame (agreement);
    // NS_ABORT_IF(true);
    return true;
  }
  ...
  if (m_muScheduler != 0
    ...
    {
      NS_LOG_DEBUG ("Asking MU scheduler");
      txFormat = m_muScheduler->NotifyAccessGranted (edca, availableTime, initialFrame);
    }

  NS_LOG_DEBUG ("m_muScheduler="<<m_muScheduler<<";txFormat="<<txFormat);
...
HeFrameExchangeManager::ReceiveBasicTrigger (const CtrlTriggerHeader& trigger, const WifiMacHeader& hdr)
{
  ...
  if (psdu != 0)
    {
      // shyam
      // uint8_t nMpdus= psdu->GetNMpdus();
      // psdu->GetHeader (nMpdus-1).SetQosEosp();
      // shyam
...
HeFrameExchangeManager::SendQosNullFramesInTbPpdu (const CtrlTriggerHeader& trigger, const WifiMacHeader& hdr)
{
  ...
  // shyam - changes --------------------------------------------------------
  // if (trigger.IsBasic ()  && m_staMac->GetTwtAgreement() && m_staMac->IsTwtPsQueueEmpty())
  // {
  //   NS_LOG_DEBUG ("This is a TWT STA and TWT PS queue is empty");
  //   header.SetPowerMgt ();
  // }
  // else
  // {
  ...
  while (tid < 8
                                       ...
                                       hdr.GetAddr2 (), txParams, ppduDuration))
    {
      ...
      if (trigger.IsBasic ()  && m_staMac->GetTwtAgreement() && m_staMac->IsTwtPsQueueEmpty())
      {
        NS_LOG_DEBUG ("Adding only one MPDU for TWT STA if queue is empty.");
        m_staMac->SetSleepAfterAck();
        break;
      }
      header.SetQosTid (++tid);
    }
  ...
  // If this is STA and within TWT SP, sleep if all queues are empty 
  // To be changed to sleep only after receiving QoS Eosp frame
  // #fixasap shyam
  if (m_staMac !=0 && m_staMac->GetTwtAgreement()) 
  {
    NS_LOG_DEBUG ("This is an STA with TWT agreement");
    if (m_staMac->IsEveryEdcaQueueEmpty ())
    {
      NS_LOG_DEBUG ("All EDCA queues are empty. Setting PHY to sleep after transmission");
      m_staMac->SetSleepAfterAck();
    }
                                     ...
                                     const WifiTxVector& txVector, bool inAmpdu)
{
      ...
      && m_txTimer.GetReason () == WifiTxTimer::WAIT_TB_PPDU_AFTER_BASIC_TF)
    {
      ...
      if (hdr.IsBlockAckReq ())
      ...
      else if (hdr.IsQosData () && !inAmpdu && hdr.GetQosAckPolicy () == WifiMacHeader::NORMAL_ACK)
        {
          ...
          if (m_mac->GetWifiRemoteStationManager ()->GetTwtAgreement(sender) && m_mac->GetWifiRemoteStationManager ()->GetTwtAnnounced (sender))
          {
            NS_LOG_DEBUG("Received QoS data frame from STA with announced TWT agreement. Setting SetTwtAwakeForSp for "<< sender <<" based on PM bit in QoS frame.");
            NS_LOG_DEBUG("GetTwtAwakeForSp for " << sender << " is currently "<<m_mac->GetWifiRemoteStationManager ()->GetTwtAwakeForSp (sender));
            m_mac->GetWifiRemoteStationManager ()->SetTwtAwakeForSp (sender, !hdr.IsPowerMgt());
            NS_LOG_DEBUG("GetTwtAwakeForSp for " << sender << " is changed to "<<m_mac->GetWifiRemoteStationManager ()->GetTwtAwakeForSp (sender));
          }
        }
  ...
  if (hdr.IsCtl ())
    {
      NS_LOG_DEBUG ("Received mpdu is CTL at he FeMan");
      if (hdr.IsAck () && m_txTimer.IsRunning ()
               ...
               && m_txTimer.GetReason () == WifiTxTimer::WAIT_BLOCK_ACKS_IN_TB_PPDU)
        {
          ...
          if (m_staExpectTbPpduFrom.empty ())
            {
              // we do not expect any other BlockAck frame
              NS_LOG_DEBUG ("we do not expect any other BlockAck frame");
              m_txTimer.Cancel ();
              m_channelAccessManager->NotifyAckTimeoutResetNow ();
              m_triggerFrame = nullptr;  // this is strictly needed for DL_MU_TF_MU_BAR only
              ...
              TransmissionSucceeded ();
              // if (m_mac->GetWifiRemoteStationManager ()->GetTwtAgreement (hdr.GetAddr2 ()))
              // {
              //   NS_LOG_DEBUG ("Last BA was received from a TWT STA, starting frame exchange again in SIFS.");
              //   // Simulator::Schedule (m_phy->GetSifs (), &HeFrameExchangeManager::StartFrameExchange, 
              //   //                       this, m_edca, m_edca->GetTxopLimit (), true );
              //   Simulator::Schedule (m_phy->GetSifs (), &HeFrameExchangeManager::StartFrameExchange, 
              //                         this, m_edca, Time::Min(), true );
               ...
               && m_txTimer.GetReason () == WifiTxTimer::WAIT_BLOCK_ACK_AFTER_TB_PPDU)
        {
          ...
          for (const auto& index : indices)
            {
              ...
              if (m_psduMap.at (staId)->GetHeader (0).IsQosData ()
                ...
                {
                  NS_ASSERT (m_psduMap.at (staId)->GetHeader (0).HasData () || m_psduMap.at (staId)->GetHeader (0).IsPowerMgt());
          ...
          if (m_staMac!=0)
          {
            NS_LOG_DEBUG ("m_staMac->SleepAfterAckIfRequired() has been called from he_feMan");
            // Only if this is an STA, call SleepAfterAckIfRequired - whether or not to sleep is decided by SleepAfterAckIfRequired
            m_staMac->SetSleepAfterAck();
            m_staMac->SleepAfterAckIfRequired();
          }
        }
      else if (hdr.IsTrigger ())
        {
          NS_LOG_DEBUG ("Received mpdu is Trigger at he FeMan");
          ...
          if (trigger.IsMuBar ())
            ...
            {
            // if (trigger.IsBasic () && m_staMac != nullptr  && m_staMac->GetTwtAgreement() && m_staMac->IsTwtPsQueueEmpty())
            // {
            //   NS_LOG_DEBUG ("This is a TWT STA and TWT PS queue is empty");
            //   m_staMac->SendOneTwtUlPpduNow ();
            // }
  ...
  // Handling TWT Accept Action frames for HE STAs
  // Detect that the frame is an Action frame 
  NS_LOG_DEBUG ("hdr.IsMgt () = "<<hdr.IsMgt ()<<"; hdr.IsAction () = "<<hdr.IsAction ()<<"; m_staMac = "<<m_staMac);
  if (hdr.IsMgt ()  && hdr.IsAction () && m_staMac != 0) 
  {
    WifiActionHeader actionHdr;
    Ptr<Packet> packet = mpdu->GetPacket ()->Copy ();
    packet->RemoveHeader (actionHdr);
    NS_LOG_DEBUG ("actionHdr.GetCategory () = "<<actionHdr.GetCategory ());
    if (actionHdr.GetCategory () == WifiActionHeader::UNPROTECTED_S1G)
    // if (actionHdr.GetCategory () == WifiActionHeader::TWT_SETUP)
    {
      NS_LOG_DEBUG("Received an UNPROTECTED_S1G Action frame at STA with MAC address: "<< m_staMac->GetAddress());
      if (actionHdr.GetAction().unprotectedS1GAction == WifiActionHeader::TWT_SETUP)
      {
        NS_LOG_DEBUG("Received an TWT_SETUP Action frame at STA with MAC address: "<< m_staMac->GetAddress());
        MgtTwtSetupHeader twtSetupHdr;
        packet->RemoveHeader (twtSetupHdr);
        Twt twtElement = twtSetupHdr.GetTwt();
        // Create a WiFi TWT agreement object from the action header
        uint8_t flowId = twtElement.GetFlowId();
        // set peeradress to the address of the AP

        Mac48Address peerMacAddress = hdr.GetAddr2();
        bool isRequestingNode = true; // TODO
        bool isImplicitAgreement = twtElement.IsImplicit();
        bool flowType = twtElement.GetFlowType();
        NS_ASSERT_MSG (flowType, "Flow type should be true (unannounced)");
        bool isTriggerBasedAgreement = twtElement.IsTriggerBased();
        // NS_ASSERT_MSG (isTriggerBasedAgreement, "isTriggerBasedAgreement should be true. Only TB TWT supported for now");
        bool isIndividualAgreement = twtElement.GetNegotiationType() == 0;
        NS_ASSERT_MSG (isIndividualAgreement, "isIndividualAgreement should be true. Only individual TWT supported for now");
        u_int16_t twtChannel = twtElement.GetTwtChannel();
        // Time wakeInterval
        uint8_t wakeIntervalExponent = twtElement.GetTwtWakeIntervalExponent();
        uint16_t wakeIntervalmantissa = twtElement.GetTwtWakeIntervalMantissa();

        // Calculation: TWT wake interval = TWT Wake Interval Mantissa x 2^(TWT Wake Interval Exponent) in micro seconds
        Time wakeInterval = MicroSeconds (wakeIntervalmantissa * pow(2, wakeIntervalExponent));
        NS_LOG_DEBUG ("wakeInterval (ms) = "<< wakeInterval.GetMilliSeconds());

        // Time nominalWakeDuration
        uint8_t nominalWakeDuration = twtElement.GetNominalWakeDuration();
        bool wakeDurationUnit = twtElement.GetWakeDurationUnit();
        NS_ASSERT_MSG (!wakeDurationUnit, "Wake duration unit should be false (256 microseconds). Only 256 microseconds supported for now");
        // Calculation: TWT nominal wake duration = TWT nominal wake duration x 256 microseconds
        Time nominalWakeDurationTime = MicroSeconds (nominalWakeDuration * 256);
        
        // Time nextTwt - received nextTwt should be added to the expected time left for next beacon generation
        Time nextTwt = MicroSeconds( twtElement.GetTargetWakeTime());
        NS_LOG_DEBUG ("Received nextTwt (us) = "<< nextTwt.GetMicroSeconds());
        Time expectedTimeOfNextBeacon = m_staMac->GetExpectedRemainingTimeTillNextBeacon();
        NS_LOG_DEBUG ("Expected time of next beacon (us) = "<< expectedTimeOfNextBeacon.GetMicroSeconds());
        Time nextTwtAdjusted = nextTwt + expectedTimeOfNextBeacon;

        NS_LOG_DEBUG ("At STA HeFeMan, STA is configured to stay awake for 500us longer than the agreed upon nominal wake duration");
        nominalWakeDurationTime = nominalWakeDurationTime + MicroSeconds (600);   // Added 100us to compensate for waking up 100us early
        NS_LOG_DEBUG ("At STA HeFeMan, STA wakes up 100 us before start of its TWT SP");
        nextTwtAdjusted = nextTwtAdjusted - MicroSeconds(100);


        // Create TWT agreement object
        WifiTwtAgreement agreement = WifiTwtAgreement (flowId, peerMacAddress, isRequestingNode, isImplicitAgreement, flowType, isTriggerBasedAgreement, isIndividualAgreement, twtChannel, wakeInterval, nominalWakeDurationTime, nextTwtAdjusted);
        
        // Print agreement 
        NS_LOG_DEBUG ("Received decoded agreement:"<<agreement);

        m_staMac->StartTwtSchedule(agreement);
...
void 
HeFrameExchangeManager::AddToPendingTwtAgreement (WifiTwtAgreement agreement)
{
  NS_LOG_FUNCTION (this);
  m_pendingTwtAgreements.push (agreement);
  NS_LOG_DEBUG ("Total pending TWT agreements now = "<< m_pendingTwtAgreements.size());
}


uint32_t 
HeFrameExchangeManager::GetPendingTwtAgreementsCount (void) const
{
  NS_LOG_FUNCTION (this);
  return m_pendingTwtAgreements.size();
}

WifiTwtAgreement 
HeFrameExchangeManager::DequeueFirstPendingTwtAgreement (void)
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG (m_pendingTwtAgreements.size() > 0, "No pending TWT agreements");
  WifiTwtAgreement agreement = m_pendingTwtAgreements.front();
  m_pendingTwtAgreements.pop();
  NS_LOG_DEBUG ("Total pending TWT agreements now = "<< m_pendingTwtAgreements.size());
  return agreement;
}
      ...
      if (m_staExpectTbPpduFrom.empty ())
        {
          ...
          //shyam - requesting channel access after receiving QoS Null Frames
      
          if (m_apMac !=0)
          {
            m_apMac->RequestBEChannelAccess();
          }
