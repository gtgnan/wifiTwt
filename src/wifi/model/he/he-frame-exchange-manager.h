...
public:
  ...
  /**
   * Add a TWT agreement to the list of pending TWT agreements.
   * 
   * \param agreement the TWT agreement to add
   */
  void AddToPendingTwtAgreement (WifiTwtAgreement agreement);

  /**
   * Return the number of pending TWT agreements.
   * 
   * \return the number of pending TWT agreements
   */
  uint32_t GetPendingTwtAgreementsCount (void) const;

  /**
   * Dequeue the first pending TWT agreement to be sent
   * 
   * \return the first pending TWT agreement to be sent
   */
  WifiTwtAgreement DequeueFirstPendingTwtAgreement (void);

  

  /**
   * Return the list of pending TWT agreements.
   * 
   * \return the list of pending TWT agreements as a vector
   */
  std::vector<WifiTwtAgreement> GetPendingTwtAgreements (void) const;
...
protected:
  ...
  /**
   * @brief Send TWT Setup frame immediately if available time is sufficient. 
   * 
   * @param agreement
   */
  void SendTwtSetupFrame (WifiTwtAgreement agreement);
...
private:
  ...
  // create a queue of TWT agreements to be sent

  std::queue <WifiTwtAgreement> m_pendingTwtAgreements; //!< list of pending TWT agreements
};
