...
public:
  ...
  /**
   * Set the STA MAC to SLEEP state - for other types of Station, do nothing
   *
   * 
   */
  virtual void SetTwtStaToSleep (void);
  /**
   * Call the STA's function NormalOrBlockAckTransmitted
   *
   */
  virtual void NotifyStaNormalOrBlockAckTransmitted (void);
  /**
  ...
  /**
   * Returns whether an Ack Tx is pending
   * 
   */
  bool IsAckTxPending (void) const;

  /**
   * Sets m_pendingAckTx 
   * 
   * \param pendingAckTx the value to set m_pendingAckTx to
   */
  void SetPendingAckTx (bool pendingAckTx);
   ...
   */
  virtual void NotifyPacketDiscarded (Ptr<const WifiMacQueueItem> mpdu);    // Moved from protected to public by Shyam
protected:
  ...
  virtual void CtsTimeout (Ptr<WifiMacQueueItem> rts, const WifiTxVector& txVector);
    ...
    /**
   * Calls StaWifiMac::isStaPsWaitingForPolledFrame () for staMac
   *
   */
  void TrySendPsPollAgain ();
private:
  /**
  ...
  bool m_pendingAckTx;                          //!< true if a Normal Ack is pending transmission
};
