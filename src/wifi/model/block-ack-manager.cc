...
BlockAckManager::StorePacket (Ptr<WifiMacQueueItem> mpdu)
{
  ...
  NS_LOG_DEBUG ("StorePacket called for mpdu to "<<mpdu->GetHeader ().GetAddr1());
...
{
  NS_LOG_FUNCTION (this<<remove<<tid<<address);
                                     ...
                                     const AgreementsI& it, const Time& now)
{
  ...
  NS_LOG_DEBUG ("MPDU has to be retransmitted. Setting retry flag in header and setting inFlight to false.");
  ...
  for (auto queueIt = it->second.second.begin (); queueIt != it->second.second.end (); ++queueIt)
    {
      if ((*queueIt)->GetHeader ().GetSequenceNumber () == mpdu->GetHeader ().GetSequenceNumber ())
        {
          NS_LOG_DEBUG ("HandleInFlightMpdu called");
...
void 
BlockAckManager::ForceAllNotifyGotBlockAck (Mac48Address recipient, uint8_t tid)
{
  NS_LOG_FUNCTION (this << recipient << tid);

  if (ExistsAgreementInState (recipient, tid, OriginatorBlockAckAgreement::ESTABLISHED))
    {
      NS_LOG_DEBUG ("Entered for "<<recipient<<" with tid = "<<tid*1);
      AgreementsI it = m_agreements.find (std::make_pair (recipient, tid));
      // const AgreementsI firstIt = m_agreements.begin();
      Mac48Address agreementMac =  m_agreements.begin()->first.first;
      uint8_t agreementTid=  m_agreements.begin()->first.second;
      NS_LOG_DEBUG ("agreementMac: "<<agreementMac<<"; agreementTid:"<<agreementTid*1);
      // auto queueIt1 = firstIt->second.second.begin ();
      
      // NS_LOG_DEBUG (*queueIt1);
      // NS_LOG_DEBUG ("Agreement is with "<< (*queueIt1)->GetHeader()->);
      ...
      for (auto queueIt = it->second.second.begin (); queueIt != it->second.second.end (); )
        {
          ...
          NS_LOG_DEBUG ("Entered for loop");
          // it->second.first.NotifyAckedMpdu (*queueIt);
          it->second.first.NotifyAckedMpduForce (*queueIt);
          Time now = Simulator::Now ();
                                    ...
                                    const std::set<uint8_t>& tids, size_t index)
{
  ...
  if (ExistsAgreementInState (recipient, tid, OriginatorBlockAckAgreement::ESTABLISHED))
    {
      NS_LOG_DEBUG ("recipient: "<<recipient<<"; tid: "<<tid*1);
      AgreementsI it = m_agreements.find (std::make_pair (recipient, tid));
          ...
          if (blockAck.IsPacketReceived (currentSeq, index))
            {
              ...
              NS_LOG_DEBUG ("Packet "<<currentSeq<<" received. HandleInFlightMpdu called with ACKNOWLEDGED");
              queueIt = HandleInFlightMpdu (queueIt, ACKNOWLEDGED, it, now);
            }
          else
            {
              NS_LOG_DEBUG ("Packet "<<currentSeq<<" not received. HandleInFlightMpdu called with TO_RETRANSMIT");
              nFailedMpdus++;
  ...
  NS_LOG_DEBUG ("nSuccessfulMpdus: "<<nSuccessfulMpdus<<"; nFailedMpdus: "<<nFailedMpdus);
...
BlockAckManager::NotifyDiscardedMpdu (Ptr<const WifiMacQueueItem> mpdu)
{
  ...
  // if (!mpdu->GetHeader ().IsRetry () && !mpdu->IsInFlight ())  // changed by shyam
  if (!mpdu->GetHeader ().IsRetry () && !mpdu->IsInFlight () && mpdu->GetHeader().GetType()!=WIFI_MAC_QOSDATA )
    {
