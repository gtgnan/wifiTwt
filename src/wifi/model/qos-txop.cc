  ...
  if (item != nullptr)
    {
      ...
      NS_LOG_DEBUG ("Peeked sequence number : "<<sequence*1);
      if (hdr.IsQosData ())
        {
          Mac48Address recipient = hdr.GetAddr1 ();
          uint8_t tid = hdr.GetQosTid ();
          NS_LOG_DEBUG ("tid : "<<tid*1);
          if (GetBaAgreementEstablished (recipient, tid)
              && !IsInWindow (sequence, GetBaStartingSequence (recipient, tid), GetBaBufferSize (recipient, tid)))
            {
              NS_LOG_DEBUG ("Current start seq: "<<GetBaStartingSequence (recipient, tid));
              NS_LOG_DEBUG ("BA Buffer Size: "<<GetBaBufferSize (recipient, tid));
