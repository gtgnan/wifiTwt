...
void
Txop::QueueFront (Ptr<Packet> packet, const WifiMacHeader &hdr)
{
  NS_LOG_FUNCTION (this << packet << &hdr);
  // remove the priority tag attached, if any
  SocketPriorityTag priorityTag;
...
{
  NS_LOG_FUNCTION (this);
  // shyam - changes to buffer TWT traffic at STA -----------------
  Ptr<StaWifiMac> m_staMac;
  m_staMac = DynamicCast<StaWifiMac> (m_mac);
  if (m_staMac != 0)
  {
    // This is an STA - if it has Trigger Based TWT, deny access
    NS_LOG_DEBUG ("This is an STA");
    if (m_staMac -> GetTwtAgreement() && m_staMac -> GetTwtTriggerBased())
    {
      NS_LOG_DEBUG ("This is an STA - Channel access denied as STA has TB TWT agreement");
      return;
    }
    if (m_staMac -> GetTwtAgreement() && !(m_staMac -> GetTwtSpNow()))
    {
      NS_LOG_DEBUG ("This is a TWT STA outside TWT SP - Channel access denied");
      return;
    }
  }
