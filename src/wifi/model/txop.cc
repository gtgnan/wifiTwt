...
Txop::ResetCw(uint8_t linkId)
{
    NS_LOG_FUNCTION(this);
    auto& link = GetLink(linkId);
    NS_LOG_DEBUG("Previous CW for link =" << link.cw);
    link.cw = GetMinCw(linkId);
    NS_LOG_DEBUG("Succeeded reset CW to " << link.cw);
...
Txop::SetAifsn(uint8_t aifsn, uint8_t linkId)
{
    ...
    NS_LOG_DEBUG ("AIFSN changed from " << GetLink(linkId).aifsn <<" to " << +aifsn <<" for link " << +linkId);
}
...
Txop::GenerateBackoff(uint8_t linkId)
{
    ...
    NS_LOG_DEBUG("backoff value ="<<backoff<<"; Generated from CW =" <<GetCw(linkId));
