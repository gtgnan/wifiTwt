...
void
OriginatorBlockAckAgreement::NotifyAckedMpduForce (Ptr<const WifiMacQueueItem> mpdu)
{
  NS_LOG_FUNCTION (this<<mpdu);
  uint16_t mpduSeqNumber = mpdu->GetHeader ().GetSequenceNumber ();
  uint16_t distance = GetDistance (mpduSeqNumber);
  NS_LOG_FUNCTION ("mpduSeqNumber: "<<mpduSeqNumber*1<<"; distance: "<<distance*1);
  ...
  // AdvanceTxWindow ();
  NS_LOG_DEBUG ("Starting sequence number before advance: " << m_txWindow.GetWinStart ());
  m_txWindow.Advance (1); //shyam
