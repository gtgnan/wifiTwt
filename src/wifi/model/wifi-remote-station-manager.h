...
// /**
//  * \brief holds one Individual TWT agreement for a remote station
//  *
//  * The state in this class is used to keep track
//  * of an individual TWT agreement status and parameters for a remote station
//  */
// struct TwtIndividualAgreement
// {
//   u_int8_t flowId;    //!< Flow ID of the TWT agreement
//   Time targetWakeTime;    //!< Target wake time of the TWT agreement
//   Time twtPeriod;    //!< TWT period of the TWT agreement
//   Time twtMinWakeDuration;    //!< TWT minimum wake duration of the TWT agreement
//   bool twtAnnounced;    //!< true for announced twt. false for unannounced
//   bool twtTriggerBased;    //!< true for trigger based twt. false for unannounced
//   bool active;    //!< True if the TWT agreement is currently active
//   EventId nextTwtSpWakeEvent;    //!< Event ID of the next TWT SP wake up
// };

// // create a map of TwtIndividualAgreement with Flow ID as key

// typedef std::unordered_map<u_int8_t, TwtIndividualAgreement > TwtIndividualAgreementMap;   //!< Map of Individual TWT agreements with Flow ID as key
...
struct WifiRemoteStationState
{
  ...
  bool m_PSM;                 //!< Flag if the remote station is currently in Power Save Mode (PS)

  // TWT state maintenance
  std::map<uint8_t, WifiTwtAgreement> m_twtAgreementMap;  // Map of TWT agreements with Flow ID as key
  uint32_t m_agreementCount;  //!< Number of TWT agreements with the remote station - should be <=8
  
  // TWT parameters - will be deprecated
  bool m_twtAnnounced;       //!< true for announced twt. false for unannounced
  bool m_twtTriggerBased;       //!< true for trigger based twt. false for unannounced
  Time m_twtPeriod;       //!< Twt period of STA
  Time m_twtMinWakeDuration;       //!< Twt minimum wake duration of STA
  // TWT states
  bool m_twtAgreement;      //!< Flag if the remote station has a TWT agreement with AP - Set to false by default
  bool m_twtActive;      //!< Flag if the remote station TWT agreement is currently active. False if suspended. Else true
  bool m_twtAwakeForSp;      //!< Flag to keep track if Sta is awake or not during TWT SP. For announced TWT, the flag indicates state for next/ongoing SP
  bool m_twtSpNow;      //!< Flag true if TWT SP is now. Does not mean STA is active. Does not mean Agreement is active. This flag is switched periodically according to agreement

  bool m_expectingBasicTf;   //!< Flag true if a Basic TF is expected by the STA now according to the AP. Used for trigger based TWT operation
};
...
public:
  ...
  /**
   * Records the current PSM state of the remote station.
   *
   * \param address the address of the station being recorded
   * \param psmEnabled whether the station is in power save
   */
  void SetPSM (Mac48Address address, bool psmEnabled);
  /**
   * Return the current (known) PSM state of the remote station.
   *
   * \param address the address of the station
   * \return the PSM state
   */
  bool GetPSM (Mac48Address address) const;
  /**
   * Sets/Unsets the current expectingBasicTf state of STA
   *
   * \param address the address of the station being set
   * \param expectingBasicTf whether the stationis expecting basic TF in DL now
   */
  void SetExpectingBasicTf (Mac48Address address, bool expectingBasicTf);
  /**
   * Return whether the STA is expecting a DL Basic TF now
   *
   * \param address the address of the station
   * \return m_expectingBasicTf
   */
  bool GetExpectingBasicTf (Mac48Address address) const;
  /**
   * Return whether twt agreement exists for this STA
   *
   * \param address the address of the station
   * \return true if twt agreement exists
   */
  bool GetTwtAgreement (Mac48Address address) const;
  /**
   * Return whether twt agreement is trigger based
   *
   * \param address the address of the station
   * \return true if twt agreement is trigger based
   */
  bool GetTwtTriggerBased (Mac48Address address) const;
  /**
   * Return whether twt agreement is announced
   *
   * \param address the address of the station
   * \return true if twt agreement is announced
   */
  bool GetTwtAnnounced (Mac48Address address) const;
  /**
   * Return whether the TWT SP for given STA is currently between TWT SP start and within min Wake duration. STA may not be awake. 
   *
   * \param address the address of the station
   * \return true if TWT SP is within min wake duration. False if Time is after min wake duration. 
   */
  bool GetTwtSpNow (Mac48Address address) const;
  /**
   * Return whether the STA is awake according to state at AP at this instant
   *
   * \param address the address of the station
   * \return true if STA is assumed to be awake
   */
  bool GetTwtAwakeForSp (Mac48Address address) const;
  /**
   * Set whether STA is awake for SP at this instant
   *
   * \param address the address of the station
   * \param twtAwakeForSp true if STA is known/assumed to be awake currently
   * \return true if STA is assumed to be awake
   */
  void SetTwtAwakeForSp (Mac48Address address, bool twtAwakeForSp);
  /**
   * Return the number of STAs currently in power save PS = 1
   *
   *
   * \return the number of STAs currently in power save
   */
  uint16_t GetNStaInPs (void) const;
  /**
   * Return the number of STAs currently expecting a DL TF
   *
   *
   * \return the number of STAs currently expecting DL TF - used for trigger based TWT
   */
  uint16_t GetNStaExpectingTf (void) const;
  /**
   ...
   * @param timeLeftTillNextBeacon  Time left till generation of next beacon at AP
   */
  void CreateTwtAgreement (uint8_t flowId, Mac48Address peerMacAddress, bool isRequestingNode, bool isImplicitAgreement, bool flowType, bool isTriggerBasedAgreement, bool isIndividualAgreement, u_int16_t twtChannel, Time wakeInterval, Time nominalWakeDuration, Time nextTwt, Time timeLeftTillNextBeacon);
  /**
   * Records the current PSM state of the remote station.
   *
   * \param address the address of the station being recorded
   * \param twtFlowId TWT agreement Flow ID 0 to 7
   * \param targetWakeTime Next TWT SP start time
   * \param twtPeriod Twt period
   * \param twtMinWakeDuration Twt Minimum wake duration
   * \param twtAnnounced true if announced TWT. False otherwise (default). 
   * \param twtTriggerBased true if trigger based TWT. False otherwise (default). 
   */
  // void SetTwtParameters (Mac48Address address, u_int8_t twtFlowId, Time targetWakeTime, Time twtPeriod, Time twtMinWakeDuration, bool twtAnnounced = false, bool twtTriggerBased = true);
  void SetTwtParameters (Mac48Address address, Time twtPeriod, Time twtMinWakeDuration, bool twtAnnounced = false, bool twtTriggerBased = true);
  /**
   * Keeps track of TWT SP. This function switches m_twtSpNow to true and schedules when to switch it to false
   *
   ...
   * \param twtMinWakeDuration Twt Minimum wake duration
   */
  void EnableTwtSpNow (uint8_t flowId, Mac48Address address, Time twtPeriod, Time twtMinWakeDuration);
  /**
   * Keeps track of TWT SP. This function switches m_twtSpNow to false
   *
   * \param flowId Twt Flow ID
   * \param address the address of the station being recorded
   */
  void DisableTwtSpNow (uint8_t flowId, Mac48Address address);
  /**
   * Update TWT Resume/Suspend state for an STA based on TWT IF
   *
   * \param address the address of the station being recorded
   * \param twtId Twt Flow ID
   * \param nextTwt Next Twt from the Twt IF. If not present, 0 is passed to this function
   */
  void UpdateFromTwtInfoFrame (Mac48Address address, u_int8_t twtId, u_int64_t nextTwt);
...
private:
  ...
  uint16_t m_nStaInPs;            //!< Current count of Associated STAs in PS
  uint16_t m_nStaExpectingBasicTf;            //!< Current count of STAs expecting DL trigger frame - used for Trigger based TWT
