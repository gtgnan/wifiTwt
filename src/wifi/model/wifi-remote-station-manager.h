...
struct WifiRemoteStationState
{
    ...
    uint32_t m_twtAgreementCount;  //!< Number of TWT agreements with the remote station - should be <=8
};
...
class WifiRemoteStationManager : public Object
{
  public:
    ...
    /**
     * begins TWT SP for given node and flowId. Function is used for both STA and AP MACs. The next TWT SP is also scheduled with this function (reschedules itself)
     *
     * \param flowId Twt Flow ID
     * \param peerMacAddress the address of the other node (AP or STA) 
     * \param twtWakeInterval Twt wake interval
     * \param twtNominalWakeDuration Twt Nominal wake duration
     */
    void EndTwtSpNow (uint8_t flowId, Mac48Address peerMacAddress, Time twtWakeInterval, Time twtNominalWakeDuration);
    /**
     * Ends TWT SP for given node and flowId. Function is used for both STA and AP MACs
     *
     * \param flowId Twt Flow ID
     * \param peerMacAddress the address of the other node (AP or STA) 
     * \param twtWakeInterval Twt wake interval
     * \param twtNominalWakeDuration Twt Nominal wake duration
     */
    void BeginTwtSpNow (uint8_t flowId, Mac48Address peerMacAddress, Time twtWakeInterval, Time twtNominalWakeDuration);
    /**
     * Returns the time left till the end of ongoing TWT SPs for the given peer MAC address. If no TWT agreements are established, an error is thrown. If no TWT SPs are ongoing, 0 is returned
     *
     * \param peerMacAddress the address of the other node (AP or STA) 
     * \return time remaining till the end of all ongoing TWT SPs. If multiple TWT SPs are ongoing, the latest ending TWT SP time is returned
     */
    Time GetTimeTillEndOfOngoingTwtSPs (Mac48Address peerMacAddress);
    /**
     * Returns the number of established TWT agreements with the given peer MAC address
     *
     * \param peerMacAddress the address of the other node (AP or STA) 
     * \return the number of established TWT agreements with the given peer MAC address
     */
