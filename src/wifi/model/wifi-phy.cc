...
WifiPhy::SetSleepMode()
{
    ...
    m_powerRestricted = false;
    m_channelAccessRequested = false;
    if (IsReceivingPhyHeader())
    {
        // PHY is decoding the PHY header of PPDU, abort current reception
        NS_LOG_DEBUG ("PHY is decoding the PHY header of PPDU, abort current reception");
        AbortCurrentReception(SLEEPING);
    }
    switch (m_state->GetState())
    {
    case WifiPhyState::TX:
