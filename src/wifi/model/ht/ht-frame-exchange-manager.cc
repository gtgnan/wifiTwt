...
{
  NS_LOG_DEBUG ("HtFrameExchangeManager:NeedSetupBlockAck called for MAC address " << recipient << " and TID " << +tid);
                                       ...
                                       Time availableTime, bool initialFrame)
{
  ...
  NS_LOG_DEBUG ("m_mpduAggregator is called from HtFrameExchangeManager");
  NS_LOG_DEBUG ("availableTime (us) = "<<availableTime.GetMicroSeconds());
          ...
          if (m_mac->GetTypeOfStation () == STA
              && (m_setQosQueueSize || hdr.IsQosEosp ()))
            {
              ...
              if (!queueSizeForTid[tid].has_value ())
                {
                  uint8_t queueSize = edca->GetQosQueueSize (tid, hdr.GetAddr1 ());
                  NS_LOG_DEBUG ("Reported Queue size: "<<1*queueSize);
                  NS_LOG_DEBUG ("Size of this MPDU (bytes): "<<1*mpdu->GetSize());
                  uint8_t thisMpduSize = static_cast<uint8_t> (std::ceil (1*mpdu->GetPacketSize() / 256.0));
                  NS_LOG_DEBUG ("Size of this MPDU (units of 256 bytes): "<<+thisMpduSize);
                  // #hardcoded value - shyam
                  if (queueSize > 0)    // hardcoded
                  {
                    queueSizeForTid[tid] = std::min (queueSize - thisMpduSize, 254);  // + 1 to account for the issue of underreporting packet sizes
                    // queueSizeForTid[tid] = std::min (queueSize + 1 - thisMpduSize, 254);  // + 1 to account for the issue of underreporting packet sizes
                  }
                  else
                  {
                    queueSizeForTid[tid] = 0;
                  }

                  if (queueSizeForTid[tid].value() == 1)
                  {
                    queueSizeForTid[tid] = 0;
                  }
                  
                  // queueSizeForTid[tid] = edca->GetQosQueueSize (tid, hdr.GetAddr1 ());     // Original
                }
              
              //shyam - to exclude MPDUs in this AMPDU from Queue size calculation 🔽
              else
              {
                NS_LOG_DEBUG ("queueSizeForTid was already computed for this A-MPDU as "<<1*queueSizeForTid[tid].value()<<"; subtracting MPDU size for each subsequent MPDU");
                uint8_t thisMpduSize = static_cast<uint8_t> (std::ceil (1.0*mpdu->GetPacketSize() / 256.0));
                NS_LOG_DEBUG ("Size of this MPDU (units of 256 bytes): "<<+thisMpduSize);
                
                queueSizeForTid[tid] = std::max(queueSizeForTid[tid].value() - thisMpduSize, 0);
                if (queueSizeForTid[tid].value() == 1 or queueSizeForTid[tid].value() == 2)
                {
                  queueSizeForTid[tid] = 0;
                  NS_LOG_DEBUG ("queueSizeForTid[tid] was 1 or 2. It is now set to zero");
                }
                NS_LOG_DEBUG ("Queue size is updated as : "<<+queueSizeForTid[tid].value());
                
                //shyam - for fixing the case where reported queue size is one less than actual
                // if (queueSizeForTid[tid].value() > 0)
                // {
                //   queueSizeForTid[tid] = queueSizeForTid[tid].value() + 1;
                // }
                // shyam
              }
              //shyam - to exclude MPDUs in this AMPDU from Queue size calculation 🆙
  ...
  if (edca->UseExplicitBarAfterMissedBlockAck () || isBar)
    {
      ...
      if (GetBaManager (tid)->NeedBarRetransmission (tid, recipient))
        {
          // Check if recipient is a STA with a TWT agreement and is currently not active
          if (m_mac->GetTypeOfStation () == AP)
          {
            NS_LOG_DEBUG ("Missed Block Ack at an AP");
            Ptr<WifiRemoteStationManager> rsm = m_mac->GetWifiRemoteStationManager ();
            if (rsm->GetTwtAgreement(recipient) && !(rsm-> GetTwtAwakeForSp (recipient)))
            {
              // Recipient is a STA with a TWT agreement and is currently not active
              NS_LOG_DEBUG ("At AP HtFrameExchangeManager, Recipient is a STA with a TWT agreement and is currently not active; do not transmit a BlockAckReq; Discard the PSDUs as failed and reset CW");
                                      ...
                                      WifiTxVector& blockAckTxVector, double rxSnr)
{
  ...
  Time ackDuration = m_phy->CalculateTxDuration (psdu, blockAckTxVector, m_phy->GetPhyBand ());
  NS_LOG_DEBUG ("Scheduling StaWifiMac::NormalOrBlockAckTransmitted after the BlockAck frame duration if it is an STA");
