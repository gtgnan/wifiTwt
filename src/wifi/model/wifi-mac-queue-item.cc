...
{
  NS_LOG_FUNCTION (this<<"Is this item queued?:"<<IsQueued ());
  NS_ASSERT (IsQueued ());
  return *m_queueIt;
}
...
void
WifiMacQueueItem::ForceSetQueueAc (AcIndex queueAc)
{
  NS_ASSERT_MSG (queueAc == AC_BE, "Function created only for AC_BE");
  m_queueAc = queueAc;
  return;
}
