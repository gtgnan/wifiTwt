...
QosFrameExchangeManager::SendCfEndIfNeeded (void)
{
  ...
  if (m_edca->GetRemainingTxop () > txDuration)
    {
      NS_LOG_DEBUG ("m_edca->GetRemainingTxop ()="<<m_edca->GetRemainingTxop ()<<"; txDuration="<<txDuration);
  ...
  if (m_edca->GetTxopLimit ().IsStrictlyPositive ())
    {
      ...
      if (!m_edca->IsTxopStarted ()
          || (backingOff && m_edca->GetRemainingTxop ().IsZero ()))
        {
          ...
          NS_LOG_DEBUG ("StartFrameExchange in QosFrameExchangeManager with txopDuration (us) ="<<txopDuration.GetMicroSeconds());
          if (m_mac->GetTypeOfStation() == TypeOfStation::STA)
          {
            NS_LOG_DEBUG ("Node is an STA");
                                     ...
                                     Time availableTime) const
{
  ...
  NS_LOG_DEBUG ("ppduDurationLimit (us) = "<<ppduDurationLimit.GetMicroSeconds());
                                                    ...
                                                    Time ppduDurationLimit) const
{
  ...
  Time remainingTwtSpTime = ppduDurationLimit;
  if (m_mac->GetTypeOfStation() == TypeOfStation::STA)
  {
    NS_LOG_DEBUG ("Node is an STA. ");
    Ptr<StaWifiMac> staMac = StaticCast<StaWifiMac> (m_mac);
    if (staMac->GetTwtAgreement() && staMac->GetTwtSpNow())
    {
      NS_LOG_DEBUG ("STA has twt agreement.");
      if (staMac->GetTwtSpNow())
      {
        remainingTwtSpTime =  staMac->GetRemainingTimeInTwtSp();
        NS_LOG_DEBUG ("Remaining time in TWT SP = "<<remainingTwtSpTime.As(Time::MS));
        NS_LOG_DEBUG ("PPDU duration: " << txTime.As (Time::MS));
      }
      else
      {
        NS_LOG_ERROR ("STA tried MPDU aggregation outside TWT SP");
        
      }

      if (remainingTwtSpTime.IsStrictlyPositive () && txTime > remainingTwtSpTime)
      {
        NS_LOG_DEBUG ("Frame will exceed TWT SP limit");
        return false;
      }
...
{
  NS_LOG_DEBUG (this);
  NS_LOG_DEBUG ("QosFrameExchangeManager::TransmissionSucceeded");

  // TODO This will be removed once no Txop is installed on a QoS station
  if (m_edca == 0)
    {
      NS_LOG_DEBUG ("m_edca = 0");
      FrameExchangeManager::TransmissionSucceeded ();
      return;
    }

  NS_LOG_DEBUG ("m_edca->GetRemainingTxop ()=>"<<m_edca->GetRemainingTxop ());
  NS_LOG_DEBUG ("m_edca->GetTxopLimit ()==>"<<m_edca->GetTxopLimit ());
  ...
  if (m_edca->GetTxopLimit ().IsStrictlyPositive ()
  ...
  else
    {
      ...
      if (m_mac->GetTypeOfStation() == TypeOfStation::STA)
      {
        NS_LOG_DEBUG ("This is a QoS STA. Transmission of last frame succeeded.");
        Ptr<StaWifiMac> staMac = DynamicCast<StaWifiMac> (m_mac);
        // NS_LOG_DEBUG ("staMac is at: "<<staMac);
        staMac->SleepAfterAckIfRequired ();
      }
      // If this is AP and there are STAs expecting Basic TF, start channel access
      // If this is AP and there is at least one frame buffered for an Active TWT STA, start channel access
      if (m_mac->GetTypeOfStation() == TypeOfStation::AP)
      {
        Ptr<ApWifiMac> apMac = DynamicCast<ApWifiMac> (m_mac);
        if (apMac->GetHeSupported ())
        {
          // This is a HE AP
          NS_LOG_DEBUG ("This is a HE AP. Transmission of last frame succeeded.");
          uint16_t nStaExpectingTf = apMac->GetWifiRemoteStationManager ()->GetNStaExpectingTf();
          uint8_t pendingTwtAgreementCount = apMac->GetPendingTwtAgreementCount ();
          if (nStaExpectingTf > 0 || pendingTwtAgreementCount > 0) 
            {
              NS_LOG_DEBUG ("Scheduling channel access Now. nStaExpectingTf = "<<1*nStaExpectingTf<<"; pendingTwtAgreementCount = "<<1*pendingTwtAgreementCount);
              apMac->RequestBEChannelAccess();
            }
        }
  ...
  if (hdr.IsQosData ())
    {
      ...
      if (hdr.IsQosEosp() && m_mac->GetTypeOfStation() == TypeOfStation::STA)
      {
        NS_LOG_DEBUG ("Node is STA and IsQosEosp =  true for this frame.");
        NS_LOG_DEBUG ("Add logic to set STA to sleep here - shyam");
