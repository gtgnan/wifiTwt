...
UdpClient::Send (void)
{
  ...
  p->AddHeader (seqTs);
  NS_LOG_INFO ("Send function called from UdpClient with packet size "<<m_size);
  NS_LOG_INFO ("Of that, 12 bytes are from seqTs header and "<<m_size-(8+4)<<" bytes are from payload");
