...
csvFileName = 'psm100Sta9May.csv'          #changeThisBeforeRun
...
stateLogStartTime_us = 10 * 1e6          # PHY state log will be imported only after this time from beginning #changeThisBeforeRun
stateLogEndTime_us = 1000 * 1e6          # PHY state log will be imported only till this time #changeThisBeforeRun
...
# print(f"{ns3_state_log_list}, {ns3_ascii_log_list}")
# logCount = 1
...
for logFilesCounter in range(logCount):
    ...
    print(f"ns3_state_log:{ns3_state_log}, ns3_ascii_log: {ns3_ascii_log}")
    ...
    for currentStaId in staIdList:
        ...
        StaNodeDeviceStringASCII = "NodeList/" + str(currentStaId) + "/DeviceList/0" 
        # logFilesCounter = 0
        awakeTime_us_this = 0
        txTime_us_this = 0
        rxTime_us_this = 0
...
dataFrameToWrite = pd.DataFrame(list(zip(index_list, processedStaId_list, totalDuration_us_list,  averageCurrent_mA_list, stateTransitions_list, bufferCurrent_list, awakeTime_us_list, txTime_us_list, rxTime_us_list)), columns= ['SimulationID', 'processedStaId', 'totalDuration_us', 'averageCurrent_mA', 'stateTransitions', 'bufferCurrent', 'awakeTime_us', 'txTime_us', 'rxTime_us'])
